<?php
	abstract class __topics_edit_forum {

		public function topic_blocking() {
			...
			$this->redirect($this->pre_lang . "/admin/forum/topics_list/" . $parent_id . "/");
		}
		...
		public function topic_edit() {
			...
			if(system_is_allowed("forum", "topic_edit", $parent))
...
END;
			...
			if(cmsController::getInstance()->getModule('users')) {
				$params['perm_panel'] = cmsController::getInstance()->getModule('users')->get_perm_panel("forum", "view", "topic_edit", $element_id);
			}
			...
			$hierarchy_type = umiHierarchyTypesCollection::getInstance()->getTypeByName("forum", "topic");
			...
			if($backup_inst = cmsController::getInstance()->getModule("backup")) {
				$params['backup_panel'] = $backup_inst->backup_panel("forum", "topic_edit_do", $element_id);
			}
			...
			$params['edit_cancel_redirect'] = $this->pre_lang . "/admin/forum/topics_list/" . $parent_id;
			return $this->parse_form("topic_add", $params);
		}
		...
		public function topic_edit_do() {
			...
			if($backup_inst = cmsController::getInstance()->getModule("backup")) {
				$backup_inst->backup_save("forum", "topic_edit_do", $element_id);
			}
			...
			if($exit_after_save) {
				...
				$this->redirect($this->pre_lang . "/admin/forum/topic_edit/{$parent_id}/{$element_id}/");
			}
		}
	};
?>