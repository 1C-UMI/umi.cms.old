<?php
	abstract class __confs_edit_forum {

		public function conf_blocking() {
...
END;
		...
		$params['cifi_headers'] = $headers_images->make_div() . $headers_images->make_element($header_pic_path);
			...
			if(cmsController::getInstance()->getModule('users')) {
				$params['perm_panel'] = cmsController::getInstance()->getModule('users')->get_perm_panel("forum", "view", "conf_edit", $element_id);
			}
			...
			if($backup_inst = cmsController::getInstance()->getModule("backup")) {
				$params['backup_panel'] = $backup_inst->backup_panel("forum", "conf_edit_do", $element_id);
			}
			...
			$params['edit_cancel_redirect'] = $this->pre_lang . "/admin/forum/confs_lists/" . $parent;
