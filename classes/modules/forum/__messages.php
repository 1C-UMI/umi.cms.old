<?php
	abstract class __messages_forum {
		public function messages_list() {
			...
			for($i = 0; $i < sizeof($result); $i++) {
				...
				if($element->getIsActive()) {
					$blocking = <<<END
						<a href="%pre_lang%/admin/forum/message_blocking/{$parent_id}/{$element_id}/0/"><img src="/images/cms/admin/%skin_path%/ico_unblock.%ico_ext%" alt="Заблокировать" title="Заблокировать" border="0" /></a>
END;
				} else {
					$blocking = <<<END
						<a href="%pre_lang%/admin/forum/message_blocking/{$parent_id}/{$element_id}/1/"><img src="/images/cms/admin/%skin_path%/ico_block.%ico_ext%" alt="Разблокировать" title="Разблокировать" border="0" /></a>
END;
				}
				$rows .= <<<END
<row>
	<col>
		<a href="%pre_lang%/admin/forum/message_edit/{$parent_id}/{$element_id}/"><b><![CDATA[$name]]></b></a> %core getTypeEditLink({$object_type_id})%
		<br /><br />
		<table border="0">
			<tr>
	...
	<col style="text-align: center;">
		<a href="%pre_lang%/admin/forum/message_edit/{$parent_id}/{$element_id}/">
			<img src="/images/cms/admin/%skin_path%/ico_edit.%ico_ext%" alt="Редактировать" title="Редактировать" />
		</a>
	</col>

	<col style="text-align: center;">
		<a href="%pre_lang%/admin/forum/message_del/{$parent_id}/{$element_id}/" commit="Вы уверены?">
...
END;
			...
			return $this->parse_form("messages", $params);
		}
...
END;
			...
			$params['rows'] = $rows;
			$params['parent_id'] = $parent_id;
			return $this->parse_form("last_messages", $params);
		}
	};
?>