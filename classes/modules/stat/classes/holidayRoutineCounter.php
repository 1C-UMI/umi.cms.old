...
class holidayRoutineCounter
{
    ...
    public static function count($start, $finish)
    {
        if (empty(self::$results[md5($start . $finish)])) {
            ...
            $r = mysql_query($q = "SELECT DATE_FORMAT(CONCAT('" . date('Y', $start) . "-', `month`, '-', `day`), '%w') AS `day_of_week` FROM `cms_stat_holidays` WHERE (`day` >= " . date('d', $start) . " AND `month` = " . date('m', $start) . " AND `day` <= " . date('d', $finish) . " AND `month` = " . date('m', $finish) . ") OR (`month` > " . date('m', $start) . " AND `month` < " . date('m', $finish) . ") HAVING `day_of_week` BETWEEN 1 AND 5");
