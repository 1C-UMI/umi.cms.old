...
class reportBuilder
{
    ...
    public function doQuery($index)
    {
        ...
        if (empty($this->filters)) {
            ...
            mysql_query($qry1 = "DROP TEMPORARY TABLE IF EXISTS `" . $tmp_table . "`");
            mysql_query($qry2 = "CREATE TEMPORARY TABLE `" . $tmp_table . "` (`id` INT, KEY `id` (`id`)) ENGINE = MEMORY");
            mysql_query($qry3 = "INSERT INTO `" . $tmp_table . "` " . $pre . " WHERE `p`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . "");
            //echo $qry2 . "\r\n" . $qry3;
        }
        ...
        foreach ($this->filters as $key => $val) {
            ...
            mysql_query("DROP TEMPORARY TABLE IF EXISTS `" . $tmp_table . "`");
            mysql_query("CREATE TEMPORARY TABLE `" . $tmp_table . "` (`id` INT, KEY `id` (`id`)) ENGINE = MEMORY");
            if ($val['name'] == 'searchEngine') {
                $qry = 'INSERT INTO `' . $tmp_table . '` ' . $pre . " INNER JOIN `cms_stat_sources` `s` ON `p`.`source_id` = `s`.`id`
                         INNER JOIN `cms_stat_sources_search` `ss` ON `ss`.`id` = `s`.`concrete_src_id`
                          ...
                          "WHERE `p`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . " AND `s`.`src_type` = 2  AND `sse`.`name` IN (" . $values . ")";
            } elseif ($val['name'] == 'searchQuery') {
                $qry = 'INSERT INTO `' . $tmp_table . '` ' . $pre . " INNER JOIN `cms_stat_sources` `s` ON `p`.`source_id` = `s`.`id`
                         INNER JOIN `cms_stat_sources_search` `ss` ON `ss`.`id` = `s`.`concrete_src_id`
                          INNER JOIN `cms_stat_sources_search_queries` `ssq` ON `ssq`.`id` = `ss`.`text_id` " .
                          ($key > 0 ? ' INNER JOIN `tmp_filtered' . ($key - 1) . '` `t` ON `t`.`id` = `p`.`id`' : '') .
                          "WHERE `p`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . " AND `s`.`src_type` = 2  AND `ssq`.`text` IN (" . $values . ")";
                ...
                " INNER JOIN `cms_stat_sources` `s` ON `p`.`source_id` = `s`.`id`
                              INNER JOIN `cms_stat_sources_sites` `ss` ON `ss`.`id` = `s`.`concrete_src_id`
                               INNER JOIN `cms_stat_sources_sites_domains` `ssd` ON `ssd`.`id` = `ss`.`domain`
                                WHERE `p`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . " AND `s`.`src_type` = 1 AND `ssd`.`name` IN (" . $values . ")";
                ...
                $qry = 'INSERT INTO `' . $tmp_table . '` ' . $pre . " INNER JOIN `cms_stat_hits` `h2` ON `h2`.`path_id` = `p`.`id`
                          INNER JOIN `cms_stat_events_collected` `ec` ON `ec`.`hit_id` = `h2`.`id`
                           INNER JOIN `cms_stat_events` `e` ON `e`.`id` = `ec`.`event_id`
                            WHERE `p`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . " AND `e`.`name` = (" . $values . ") AND `e`.`type` = 1";
            }
            //echo $qry, '<br>';
