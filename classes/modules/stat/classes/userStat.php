...
class userStat extends simpleStat
{
    ...
    public function get()
    {
        ...
        if ((int)$this->params['user_id']) {
            ...
            mysql_query("SET @visits_count = (SELECT COUNT(*) AS `visits_cnt` FROM `cms_stat_paths` `p`
                         INNER JOIN `cms_stat_users` `u` ON `p`.`user_id` = `u`.`id`
                          WHERE `p`.`host_id` = " . $this->host_id . " AND `p`.`user_id` = " . (int)$this->params['user_id'] . ")");

            $tmp = $this->simpleQuery("SELECT @visits_count AS `cnt`");
            $result['visit_count'] = $tmp[0]['cnt'];
            unset($tmp);

            mysql_query("SET @first_path = (SELECT MIN(`id`) FROM `cms_stat_paths` WHERE `user_id` = " . (int)$this->params['user_id'] . ")");
            ...
            $tmp = $this->simpleQuery("SELECT  ( UNIX_TIMESTAMP(MAX(`date`)) - UNIX_TIMESTAMP(MIN(`date`)) ) / (COUNT(*) - 1) / 3600 / 24  AS `days` FROM `cms_stat_paths`
                                         WHERE `date` BETWEEN " . $this->getQueryInterval() . " AND `host_id` = " . $this->host_id . " AND `user_id` = " . (int)$this->params['user_id']);
            ...
            $tmp = $this->simpleQuery("SELECT COUNT(*) AS `abs`, COUNT(*) / @visits_count * 100 AS `rel`  FROM `cms_stat_hits` `h`
                                         INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                                          LEFT JOIN `cms_stat_hits` `h2` ON `h2`.`path_id` = `h`.`path_id` AND `h2`.`number_in_path` = 2
                                           WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `h`.`number_in_path` = 1 AND `h2`.`id` IS NULL AND `p`.`user_id` = " . (int)$this->params['user_id'] . " AND `p`.`host_id` = " . $this->host_id);
            $result['refuse_frequency'] = $tmp[0];
            unset($tmp);

            mysql_query("SET @hits_count = (SELECT COUNT(*) FROM `cms_stat_hits` `h`
                         INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                          INNER JOIN `cms_stat_pages` `pg` ON `pg`.`id` = `h`.`page_id`
                           WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`user_id` = " . (int)$this->params['user_id'] . " AND `p`.`host_id` = " . $this->host_id . ")");
            ...
            $tmp = $this->simpleQuery("SELECT UNIX_TIMESTAMP(`p`.`date`) AS `last_visit`, `s`.`concrete_src_id`, `s`.`src_type` FROM `cms_stat_paths` `p`
                                         INNER JOIN `cms_stat_sources` `s` ON `p`.`source_id` = `s`.`id`
                                          WHERE `p`.`user_id` = " . (int)$this->params['user_id'] . " AND `p`.`host_id` = " . $this->host_id . "
                                           ORDER BY `p`.`id` DESC
                                            LIMIT 1");
            ...
            mysql_query("SET @last_path = (SELECT `p`.`id` FROM `cms_stat_paths` `p`
                         WHERE `p`.`user_id` = " . (int)$this->params['user_id'] . " AND `p`.`host_id` = " . $this->host_id . "
                          ORDER BY `p`.`id` DESC
                           LIMIT 1)");
            ...
            mysql_query("DROP TEMPORARY TABLE IF EXISTS `tmp_collected_events`");
            mysql_query("CREATE TEMPORARY TABLE `tmp_collected_events` (`id` INT, `name` CHAR(255), `description` CHAR(255), `profit` float(9,2), KEY `id` (`id`)) ENGINE = MEMORY");

            mysql_query("INSERT INTO `tmp_collected_events` SELECT IFNULL(`e2`.`id`, `e`.`id`), IFNULL(`e2`.`name`, `e`.`name`), IFNULL(`e2`.`description`, `e`.`description`), IFNULL(`e2`.`profit`, `e`.`profit`) FROM `cms_stat_hits` `h`
                             INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                               ...
                               INNER JOIN `cms_stat_events` `e` ON `e`.`id` = `ec`.`event_id`
                                LEFT JOIN `cms_stat_events_rel` `er` ON `er`.`metaevent_id` = `e`.`id`
                                 LEFT JOIN `cms_stat_events` `e2` ON `e2`.`id` = `er`.`event_id`
                                  WHERE `p`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`user_id` = " . (int)$this->params['user_id'] . " AND `e`.`type` = 1 AND `p`.`host_id` = " . $this->host_id);
            ...
            mysql_query("DROP TEMPORARY TABLE IF EXISTS `tmp_collected_labels`");
            mysql_query("CREATE TEMPORARY TABLE `tmp_collected_labels` (`id` INT, `name` CHAR(255), `description` CHAR(255), KEY `id` (`id`)) ENGINE = MEMORY");

            mysql_query("INSERT INTO `tmp_collected_labels` SELECT IFNULL(`e2`.`id`, `e`.`id`), IFNULL(`e2`.`name`, `e`.`name`), IFNULL(`e2`.`description`, `e`.`description`) FROM `cms_stat_hits` `h`
                             INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                              INNER JOIN `cms_stat_events_collected` `ec` ON `ec`.`hit_id` = `h`.`id`
                               INNER JOIN `cms_stat_events` `e` ON `e`.`id` = `ec`.`event_id`
                                LEFT JOIN `cms_stat_events_rel` `er` ON `er`.`metaevent_id` = `e`.`id`
