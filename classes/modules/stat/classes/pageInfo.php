...
class pageInfo extends simpleStat
{
    ...
    public function get()
    {
        ...
        $result['source'] = $this->simpleQuery("(SELECT COUNT(*) AS `cnt`, 'sites' AS `type`, `d`.`name` AS `name` FROM `cms_stat_hits` `h`
                                     ...
                                     INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
                                      INNER JOIN `cms_stat_sources_sites` `ss` ON `s`.`concrete_src_id` = `ss`.`id`
                                       INNER JOIN `cms_stat_sources_sites_domains` `d` ON `d`.`id` = `ss`.`domain`
                                        WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `s`.`src_type` = 1 AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `h`.`number_in_path` = 1
                                         GROUP BY `d`.`id`)
                                     ...
                                     INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
                                      INNER JOIN `cms_stat_sources_search` `ss` ON `ss`.`id` = `s`.`concrete_src_id`
                                       INNER JOIN `cms_stat_sources_search_engines` `sse` ON `sse`.`id` = `ss`.`engine_id`
                                        WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `s`.`src_type` = 2 AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `h`.`number_in_path` = 1
                                         GROUP BY `sse`.`id`)
                                    ...
                                    INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                                     INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
                                      INNER JOIN `cms_stat_sources_pr` `pr` ON `pr`.`id` = `s`.`concrete_src_id`
                                       WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `s`.`src_type` = 3 AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `h`.`number_in_path` = 1
                                        GROUP BY `pr`.`id`)
                                    ...
                                    INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                                     INNER JOIN `cms_stat_sources` `s` ON `s`.`id` = `p`.`source_id`
                                      INNER JOIN `cms_stat_sources_ticket` `t` ON `t`.`id` = `s`.`concrete_src_id`
                                       WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `s`.`src_type` = 4 AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `h`.`number_in_path` = 1
                                        GROUP BY `t`.`id`)
                                    ...
                                    INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                                     WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `h`.`number_in_path` = 1 AND `p`.`source_id` = 0
                                      GROUP BY `h`.`page_id`)
        ...
        $tmp = $this->simpleQuery("SELECT COUNT(*) AS `abs`, COUNT(*) / @all * 100 AS `rel` FROM `cms_stat_hits` `h`
                                                 INNER JOIN `cms_stat_pages` `p` ON `p`.`id` = `h`.`page_id`
                                                  WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `p`.`host_id` = " . $this->host_id . "
                                                   GROUP BY `p`.`id`");
        if (!isset($tmp[0])) {
        ...
        $tmp = $this->simpleQuery("SELECT COUNT(*) AS `abs` FROM `cms_stat_hits` `h`
                                                 INNER JOIN `cms_stat_pages` `p` ON `p`.`id` = `h`.`page_id`
                                                  WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `h`.`number_in_path` = 1 AND `p`.`host_id` = " . $this->host_id . " AND `h`.`page_id` = " . (int)$this->params['page_id'] . "
                                                   GROUP BY `p`.`id`");
        if (!isset($tmp[0])) {
            $tmp[0] = array('abs' => 0);
        }
        ...
        $tmp = $this->simpleQuery("SELECT COUNT(*) AS `abs` FROM `cms_stat_hits` `h`
                                     INNER JOIN `tmp_paths_out` `t` ON `h`.`path_id` = `t`.`path_id` AND `h`.`number_in_path` = `t`.`level`
                                      INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                                       WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . " AND `h`.`page_id` = " . (int)$this->params['page_id'] . "
                                        GROUP BY `h`.`page_id`");
        ...
        $tmp = $this->simpleQuery("SELECT COUNT(*) AS `abs` FROM `cms_stat_hits` `h`
                                     LEFT JOIN `cms_stat_hits` `h2` ON `h2`.`path_id` = `h`.`path_id` AND `h2`.`number_in_path` = `h`.`number_in_path` + 1
                                      INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                                       WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . " AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `h2`.`id` IS NULL AND `h`.`number_in_path` = 1");
                                                                  ...
                                                                  INNER JOIN `cms_stat_events` `e` ON `e`.`id` = `ec`.`event_id`
                                                                   LEFT JOIN `cms_stat_events_rel` `er` ON `er`.`metaevent_id` = `e`.`id`
                                                                    LEFT JOIN `cms_stat_events` `e2` ON `e2`.`id` = `er`.`event_id`
                                                                     INNER JOIN `cms_stat_pages` `p` ON `p`.`id` = `h`.`page_id`
                                                                      WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . " AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `e`.`type` = 1");
                                                                      ...
                                                                      INNER JOIN `cms_stat_events_collected` `ec` ON `ec`.`hit_id` = `h2`.`id`
                                                                       INNER JOIN `cms_stat_events` `e` ON `e`.`id` = `ec`.`event_id`
                                                                        LEFT JOIN `cms_stat_events_rel` `er` ON `er`.`metaevent_id` = `e`.`id`
                                                                         LEFT JOIN `cms_stat_events` `e2` ON `e2`.`id` = `er`.`event_id`
                                                                          INNER JOIN `cms_stat_pages` `p` ON `p`.`id` = `h2`.`page_id`
                                                                           WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `h`.`page_id` = " . (int)$this->params['page_id'] . " AND `h`.`id` = (SELECT MIN(`h2`.`id`) FROM `cms_stat_hits` `h2` WHERE `h2`.`page_id` = `h`.`page_id` AND `path_id` = `h`.`path_id`) AND `e`.`type` = 1 AND `p`.`host_id` = " . $this->host_id);
