...
class visitDeep extends simpleStat
{
    ...
    {
        mysql_query("DROP TEMPORARY TABLE IF EXISTS `tmp_visit_deep`");
        mysql_query("CREATE TEMPORARY TABLE `tmp_visit_deep` (`lvl` INT) ENGINE = MEMORY");

        mysql_query("INSERT INTO `tmp_visit_deep` SELECT MAX(`number_in_path`) AS `level` FROM `cms_stat_hits` `h`
                         INNER JOIN `cms_stat_paths` `p` ON `p`.`id` = `h`.`path_id`
                          WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `p`.`host_id` = " . $this->host_id . "
                           GROUP BY `h`.`path_id`");

        return $this->simpleQuery("SELECT COUNT(*) AS `cnt`, IF(`lvl` > 10, IF(`lvl` > 20, IF(`lvl` > 30, IF(`lvl` > 40, IF(`lvl` > 50, 51, 41), 31), 21), 11), `lvl`) AS `level` FROM `tmp_visit_deep`
                                     GROUP BY `level`");
    }
