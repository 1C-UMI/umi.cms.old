...
class pageNext extends simpleStat
{
	...
	if(!$this->params['page_uri']) {
		...
		$sql = "SELECT id FROM cms_stat_pages WHERE uri = '" . mysql_real_escape_string($this->params['page_uri']) . "' AND host_id = '" . (int) $this->host_id . "' LIMIT 1";
		list($page_id) = mysql_fetch_row(mysql_query($sql));
	}
        ...
        return $this->simpleQuery("SELECT COUNT(*) AS `abs`, `p`.`uri`, `p`.`id` FROM `cms_stat_hits` `h`
                                                     INNER JOIN `cms_stat_hits` `h2` ON `h2`.`prev_page_id` = `h`.`page_id` AND `h2`.`number_in_path` = `h`.`number_in_path` + 1 AND `h2`.`path_id` = `h`.`path_id`
                                                      INNER JOIN `cms_stat_pages` `p` ON `p`.`id` = `h2`.`page_id`
                                                       WHERE `h`.`date` BETWEEN " . $this->getQueryInterval() . " AND `h`.`page_id` = " . (int) $page_id . " AND `p`.`host_id` = " . $this->host_id . "
                                                        GROUP BY `h2`.`page_id`
                                                         ORDER BY `abs` DESC");
    }
}
