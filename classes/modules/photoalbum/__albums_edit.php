<?php
	abstract class __albums_edit_photoalbum {
		...
		public function album_edit() {
			...
			if(system_is_allowed("photoalbum", "album_edit"))
...
END;
			...
			if(cmsController::getInstance()->getModule('users')) {
				$params['perm_panel'] = cmsController::getInstance()->getModule('users')->get_perm_panel("photoalbum", "view", "album_edit", $element_id);
			}
			...
			$hierarchy_type = umiHierarchyTypesCollection::getInstance()->getTypeByName("photoalbum", "album");
			...
			if($backup_inst = cmsController::getInstance()->getModule("backup")) {
				$params['backup_panel'] = $backup_inst->backup_panel("photoalbum", "album_edit_do", $element_id);
			}
			...
			$params['albom_users'] = putSelectBox_assoc(umiObjectsCollection::getInstance()->getGuidedItems($user_object_type), $element->getValue("user_id"), false);
			...
			$params['method'] = "album_edit_do";
			$params['edit_cancel_redirect'] = $this->pre_lang . "/admin/photoalbum/albums_list/" . $parent . "/";
			return $this->parse_form("album_add", $params);
		}


		public function album_edit_do() {
			...
			if($backup_inst = cmsController::getInstance()->getModule("backup")) {
				$backup_inst->backup_save("forum", "conf_edit_do", $element_id);
			}
			...
			if($exit_after_save) {
				...
				$this->redirect($this->pre_lang . "/admin/photoalbum/album_edit/" . $element_id . "/");
			}
		}
