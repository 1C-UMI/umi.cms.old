<?php
abstract class __rubrics_edit_news {
	public function edit_list() {
		...
		$this->navibar_push("%nav_news_rubric_edit% (" . $element->getName() . ")", "/admin/news/edit_list/" . $parent_id . "/" . $element_id);
...
END;
		...
		if(cmsController::getInstance()->getModule('users')) {
			$params['perm_panel'] = cmsController::getInstance()->getModule('users')->get_perm_panel("news", "view", "edit_rubric", $element_id);
		}
		...
		if($backup_inst = cmsController::getInstance()->getModule("backup")) {
			$params['backup_panel'] = $backup_inst->backup_panel("news", "edit_list_do", $element_id);
		}


		$params['parent_id'] = $parent_id;
		$params['rubric_id'] = $element_id;
		$params['method'] = "edit_list_do";
	...
	public function edit_list_do() {
		...
		if($backup_inst = cmsController::getInstance()->getModule("backup")) {
			$backup_inst->backup_save("news", "edit_list_do", $element_id);
		}
