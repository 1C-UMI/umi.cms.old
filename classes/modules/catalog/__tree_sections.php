...
abstract class __catalog_tree_sections {
	public function tree_section_add() {
		$params = Array();
		$this->load_forms();
		...
		$this->navibar_push("%nav_catalog_section_add%", "/admin/catalog/tree_section_add/" . $parent_section_id);
		...
		if(cmsController::getInstance()->getModule('users')) {
			$files_panel = cmsController::getInstance()->getModule('users')->get_perm_panel_adding("catalog", Array("section_view", "section_edit"));
		...
		if(cmsController::getInstance()->getModule('data')) {
			$type_id = umiObjectTypesCollection::getInstance()->getBaseType("catalog", "category");
			$params['data_field_groups'] = cmsController::getInstance()->getModule('data')->renderEditableGroups($type_id);
		}

		$hierarchy_type = umiHierarchyTypesCollection::getInstance()->getTypeByName("catalog", "category");
		...
		if(cmsController::getInstance()->getModule('users')) {
			$params['perm_panel'] = cmsController::getInstance()->getModule('users')->get_perm_panel("catalog", "category", "edit_section");
		}
		...
		$params['title'] = "";
		$params['method'] = "tree_section_add_do";
		...
		return $this->parse_form("tree_section_add", $params);
	}

	public function tree_section_add_do() {
		...
		if($_REQUEST['exit_after_save']) {
			...
			$this->redirect($this->pre_lang . "/admin/catalog/tree_section_edit/" . $parent_id . "/" . $element_id . "/");
		}
	}


	public function tree_section_edit() {
		$params = Array();
		$this->load_forms();
		$this->sheets_set_active("tree");
		$parent_id = (int) $_REQUEST['param0'];
		...
		$this->navibar_push("%nav_catalog_section_edit% (" . $element->getName() . ")", "/admin/catalog/tree_section_edit/" . $parent_id . "/" . $element_id);
...
//var_dump($menu_ua_images); die;
		...
		if(cmsController::getInstance()->getModule('users')) {
			$params['perm_panel'] = cmsController::getInstance()->getModule('users')->get_perm_panel("catalog", "category", "edit_section", $element_id);
		}

		if($backup_inst = cmsController::getInstance()->getModule("backup")) {
			$params['backup_panel'] = $backup_inst->backup_panel("catalog", "tree_section_edit_do", $element_id);
		}



		$params['section_id'] = $element_id;
		$params['parent_section_id'] = $parent_id;
		$params['method'] = "tree_section_edit_do";
	...
	public function tree_section_edit_do() {
		global $HTTP_POST_FILES;
		...
		if($backup_inst = cmsController::getInstance()->getModule("backup")) {
			$backup_inst->backup_save("catalog", "tree_section_edit_do", $element_id);
		}
