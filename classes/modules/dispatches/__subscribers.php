<?php
	abstract class __subscribers_subscribers {
		public function subscribers_list() {
			...
			if ($iDispId) {
				...
				if ($oDispObj instanceof umiObject) {
					...
					// navibar
					$this->navibar_back();
					$this->navibar_push("Список подписчиков", "{$this->pre_lang}/admin/dispatches/subscribers_list/");
					$this->navibar_push("%nav_subscribers_list% \"".$oDispObj->getName()."\"");
				}
			}
		...
		private function renderSubscriber($iSbsId, $iSbsNum=0) {
			...
			$params = array();
			if ($oSubscriber instanceof iUmiObject) {
				...
				$params['sbs_status'] = ($oSubscriber->getValue('uid')? "зарегистрированный пользователь": "Гость");
