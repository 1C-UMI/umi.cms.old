...
abstract class __faq_category_edit {
	public function category_blocking() {
	...
	public function category_edit() {
		...
		$this->navibar_push("%nav_cat_edit%");
...
END;
		...
		if(cmsController::getInstance()->getModule('users')) {
			$params['perm_panel'] = cmsController::getInstance()->getModule('users')->get_perm_panel("faq", "view", "category_edit", $element_id);
		}

		if(system_is_allowed("faq", "category_edit", $element_id))
		...
		if($backup_inst = cmsController::getInstance()->getModule("backup")) {
			$params['backup_panel'] = $backup_inst->backup_panel("faq", "category_edit_do", $element_id);
		}


		$params['parent_id'] = $parent_id;
		$params['element_id'] = $element_id;
		$params['method'] = "category_edit_do";
		$params['edit_cancel_redirect'] = $this->pre_lang . "/admin/faq/categories_list/" . $parent_id . "/";
	...
	public function category_edit_do() {
		...
		if($backup_inst = cmsController::getInstance()->getModule("backup")) {
			$backup_inst->backup_save("faq", "category_edit_do", $element_id);
		}
