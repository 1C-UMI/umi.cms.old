...
abstract class __faq_project_edit {
	public function project_blocking() {
		$element_id = (int) $_REQUEST['param0'];
		$is_active = (int) $_REQUEST['param1'];
	...
	public function project_edit() {
		...
		$this->navibar_back();
		$this->navibar_push("%nav_proj_edit% (" . $element->getName() . ")", "/admin/faq/project_edit/" . $parent_id . "/" . $element_id);
		...
		$params['meta_description'] = 	$element->getValue("meta_descriptions");
...
END;
		...
		$hierarchy_type = umiHierarchyTypesCollection::getInstance()->getTypeByName("faq", "project");
		...
		if(cmsController::getInstance()->getModule('users')) {
			$params['perm_panel'] = cmsController::getInstance()->getModule('users')->get_perm_panel("faq", "view", "project_edit", $element_id);
		}

		if(system_is_allowed("faq", "project_edit", $element_id))
		...
		if($backup_inst = cmsController::getInstance()->getModule("backup")) {
			$params['backup_panel'] = $backup_inst->backup_panel("faq", "project_edit_do", $element_id);
		}


		$params['parent_id'] = $parent_id;
		$params['element_id'] = $element_id;
		$params['method'] = "project_edit_do";
		$params['edit_cancel_redirect'] = $this->pre_lang . "/admin/faq/projects_list/" . $parent_id . "/";
		return $this->parse_form('project_edit', $params);
	}

	public function project_edit_do() {
		...
		if($backup_inst = cmsController::getInstance()->getModule("backup")) {
			$backup_inst->backup_save("faq", "project_edit_do", $element_id);
		}
		...
		if($exit_after_save) {
			...
			$this->redirect($this->pre_lang . "/admin/faq/project_edit/" . $parent_id . "/" . $element_id . "/");
		}
	}
}
?>