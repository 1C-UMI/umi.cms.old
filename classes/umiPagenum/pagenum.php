<?php

	class umiPagenum implements iPagenum {
		public static $max_pages = 5;

		public static function generateNumPage($total, $per_page, $template = "default", $varName = "p") {
			if(!$template) $template = "default";
			if(!$varName) $varName = "p";
			list($template_block, $template_block_empty, $template_item, $template_item_a, $template_quant, $template_tobegin, $template_tobegin_a, $template_toend, $template_toend_a, $template_toprev, $template_toprev_a, $template_tonext, $template_tonext_a) = def_module::loadTemplates("tpls/numpages/{$template}.tpl", "pages_block", "pages_block_empty", "pages_item", "pages_item_a", "pages_quant", "pages_tobegin", "pages_tobegin_a", "pages_toend", "pages_toend_a", "pages_toprev", "pages_toprev_a", "pages_tonext", "pages_tonext_a");

			if(($total <= 0) || ($total < $per_page)) {
				return $template_block_empty;
			}


			$key = $varName;
			$page_current = (int) $_REQUEST[$key];

			$params = $_GET;
			if(array_key_exists($key, $params)) {
				unset($params[$key]);
			}
			unset($params['path']);


			$max_pages = self::$max_pages;

			$block_arr = Array();

			$pages = "";
			$pages_count = ceil($total / $per_page);
			if(!$pages_count) $pages_count = 1;

			$q = (sizeof($params)) ? "&amp;" . str_replace("&", "&amp;", http_build_query($params)) : "";
			$q = str_replace("%", "&#037;", $q);



			for($i = 0; $i < $pages_count; $i++) {
				$line_arr = Array();

				$n = $i + 1;

				if(($page_current - self::$max_pages) > $i) continue;

				if(($page_current + $max_pages) < $i) break;

				$tpl = ($i == $page_current) ? $template_item_a : $template_item;

				$link = "?{$key}={$i}" . $q;

				$line_arr['link'] = $link;
				$line_arr['num'] = $n;
				$line_arr['quant'] = ($i < ($pages_count - 1)) ? $template_quant : "";

				$pages .= def_module::parseTemplate($tpl, $line_arr);
			}


			$block_arr['pages'] = $pages;
			
			$block_arr['tobegin'] = ($page_current == 0 || $pages_count <= 1) ? $template_tobegin_a : $template_tobegin;
			$block_arr['toprev']  = ($page_current == 0 || $pages_count <= 1) ? $template_toprev_a  : $template_toprev;

			$block_arr['toend'] =  ($page_current == ($pages_count - 1) || $pages_count <= 1) ? $template_toend_a : $template_toend;
			$block_arr['tonext'] = ($page_current == ($pages_count - 1) || $pages_count <= 1) ? $template_tonext_a : $template_tonext;
			
			$block_arr['tobegin_link'] = "?{$key}=0" . $q;
			$block_arr['toprev_link'] = "?{$key}=" . ($page_current -1)  . $q;
			$block_arr['toend_link'] = "?{$key}=" . ($pages_count - 1) . $q;
			$block_arr['tonext_link'] = "?{$key}=" . ($page_current + 1) . $q;

			return def_module::parseTemplate($template_block, $block_arr);
		}

		public static function generateOrderBy($fieldName, $type_id, $template = "default") {
			if(!$template) $template = "default";
			list($template_block, $template_block_a) = def_module::loadTemplates("tpls/numpages/{$template}.tpl", "order_by", "order_by_a");

			if(!($type = umiObjectTypesCollection::getInstance()->getType($type_id))) {
				return "";
			}


			$block_arr = Array();

			if(($field_id = $type->getFieldId($fieldName)) || ($fieldName == "name")) {
				$params = $_GET;
				unset($params['path']);
				$tpl = (array_key_exists($fieldName, $params['order_filter'])) ? $template_block_a : $template_block;
				unset($params['order_filter']);
				$params['order_filter'][$fieldName] = 1;

				$q = (sizeof($params)) ? "&amp;" . str_replace("&", "&amp;", http_build_query($params)) : "";
				$q = str_replace("%", "&#037;", $q);

				$block_arr['link'] = "?" . $q;


				if($fieldName == "name") {
					$block_arr['title'] = "Название";
				} else {
					$block_arr['title'] = umiFieldsCollection::getInstance()->getField($field_id)->getTitle();
				}

				return def_module::parseTemplate($tpl, $block_arr);
			}
			return "";
		}
	};
?>