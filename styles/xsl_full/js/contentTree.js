...
contentTree.prototype.sendRequest = function (action, param, handler) {
	...
	var rnd = Math.round(Math.random() % 10 * 100);

	var url = this.pre_lang + "/admin/content/json_" + action + "/" + requestId + "/?rnd=" + rnd + "&requestParam=" + param;
...
contentTree.prototype.insert = function (newParentId, prevNodeId, parentDomain, domainName) {
	...
	if(typeof next_page.id == "undefined") {
		...
		var url = this.pre_lang + "/admin/content/json_move/?id=" + page.id + "&rel=" + newParentId + "&before=" + next_page.id + "&domain=" + domainName;
	}
...
contentTree.prototype.copyElement = function (elementId, domain, mode) {
	var url = this.pre_lang + "/admin/content/json_copy/?id=" + elementId + "&domain=" + domain;
	url += (mode) ? "&copyAll=1" : "&copyAll=0";
...
contentTree.prototype.cloneElement = function (elementId, domain, mode) {
	var url = this.pre_lang + "/admin/content/json_copy/?id=" + elementId + "&cloneIt=1&domain=" + domain;
	url += (mode) ? "&copyAll=1" : "&copyAll=0";
...
contentTree.prototype.reportCopy = function (pageId, response) {
	...
	var root = this.getPage(rel_id, response.domain);
	var placer = root.placer_childs;

	var child = response;
	var page = new contentTreePage(this, child.title, child.id, child.rel, child.childs_count, child.prev_page_id, child.domain, child.module, child.method, child.add_link, child.edit_link, true, child.is_active, child.is_visible, child.view_link, child.type_title);
	page.draw(placer);
	root.childs_count += 1;
